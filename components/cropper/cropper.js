Component({
  ...
  methods: {
    ...
    _click(event, start = false) {
      ...
      this._draw(() => {
        let x = event.detail ? event.detail.x : event.touches ? event.touches[0].clientX : 0;
        let y = event.detail ? event.detail.y : event.touches ? event.touches[0].clientY : 0;
